...
final class DocumentRepository
{
    ...
    {
        $this->pdo->prepare(
            'UPDATE documents SET collection_id = :c, parent_id = :p, title = :t, content_markdown = :m, content_html = :h, updated_by = :u WHERE id = :id'
        )->execute(['c' => $collectionId, 'p' => $parentId, 't' => $title, 'm' => $markdown, 'h' => $html, 'u' => $userId, 'id' => $id]);
    }

    public function softDelete(int $id): void
    {
        $this->pdo->prepare('UPDATE attachments SET document_id = NULL WHERE document_id = :id')->execute(['id' => $id]);
        $this->pdo->prepare('UPDATE documents SET parent_id = NULL WHERE parent_id = :id')->execute(['id' => $id]);
        $this->pdo->prepare('UPDATE documents SET deleted_at = NOW() WHERE id = :id')->execute(['id' => $id]);
    }
    ...
    public function userPermission(int $documentId, int $userId): ?string
    {
        $stmt = $this->pdo->prepare(
            'SELECT permission FROM document_permissions WHERE document_id = :document_id AND user_id = :user_id LIMIT 1'
        );
    ...
    public function permissions(int $documentId): array
    {
        $stmt = $this->pdo->prepare(
    ...
    public function grantPermission(int $documentId, int $userId, string $permission): void
    {
        $stmt = $this->pdo->prepare(
            'INSERT INTO document_permissions (document_id, user_id, permission)
             VALUES (:document_id, :user_id, :permission)
             ON DUPLICATE KEY UPDATE permission = VALUES(permission), updated_at = CURRENT_TIMESTAMP'
        );
        $stmt->execute(['document_id' => $documentId, 'user_id' => $userId, 'permission' => $permission]);
    }

    public function revokePermission(int $documentId, int $userId): void
    {
